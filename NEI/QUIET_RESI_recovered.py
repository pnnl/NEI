...
df0['out.site_energy.total.energy_consumption.kwh'].dropna().plot(kind='hist', bins=30, edgecolor='black', figsize=(10, 6))
df0['in.ashrae_iecc_climate_zone_2004'].dropna().plot(kind='hist', bins=30, edgecolor='black', figsize=(10, 6))
...
# Define the colors based on the year
color_mapping = {
    '<1940': '#F9BB0E',
    '1940s': '#F9BB0E',
    '1950s': '#F9BB0E',
...
colors = [color_mapping[vintage] for vintage in vintage_counts_df['in.vintage']]
...
plt.bar(vintage_counts_df['in.vintage'], vintage_counts_df['percentage'], color=colors)#, edgecolor='black')
...
# Calculate means
pre_2000_mean = pre_2000['oitc'].mean()
post_2000_mean = post_2000['oitc'].mean()

# Create subplots
fig, axs = plt.subplots(1, 2, figsize=(15, 6), sharey=True)

# Pre 2000 histogram
axs[0].hist(pre_2000['oitc'].dropna(), bins=30, density=True, color='#F9BB0E') #no outlines of bars within histogram
#axs[0].hist(pre_2000['oitc'].dropna(), bins=30, edgecolor='black', density=True, color='#F9BB0E') #outlines
axs[0].axvline(pre_2000_mean, color='black', linestyle='--', linewidth=1.5)  # Add mean line
...
# Post 2000 histogram
axs[1].hist(post_2000['oitc'].dropna(), bins=30, density=True, color='#A90D8B') #no outlines
#axs[1].hist(post_2000['oitc'].dropna(), bins=30, edgecolor='black', density=True, color='#A90D8B') #outlines
axs[1].axvline(post_2000_mean, color='black', linestyle='--', linewidth=1.5)  # Add mean line
...
#another option: three subplots:
# Create a figure with 3 subplots
fig, axs = plt.subplots(1, 3, figsize=(22, 6), sharey=False)  # sharey=False to allow different y scales

# Bar plot for vintage distribution
axs[0].bar(vintage_counts_df['in.vintage'], vintage_counts_df['percentage'], color=colors)  # With black outline
axs[0].set_title('Building Vintage Distribution in ResStock')
axs[0].set_xlabel('in.vintage')
axs[0].set_ylabel('Percentage of ResStock Entries')
axs[0].yaxis.set_major_formatter(PercentFormatter(decimals=0))
axs[0].set_xticklabels(vintage_counts_df['in.vintage'], rotation=45)

# Pre 2000 histogram
axs[1].hist(pre_2000['oitc'].dropna(), bins=30, density=True, color='#F9BB0E')  # no outlines
axs[1].axvline(pre_2000_mean, color='black', linestyle='--', linewidth=1.5)  # Add mean line
axs[1].set_title('Histogram of OITC for Buildings Built Pre 2000')
axs[1].set_xlabel('OITC')
axs[1].set_ylabel('Percent within vintage')  # Changed to Density for clarity with density=True
axs[1].set_ylim(0, 0.25)
axs[1].yaxis.set_major_formatter(PercentFormatter(xmax=1,decimals = 0))  # Set y-axis as percentage
axs[1].set_xlim(20, 45)

# Post 2000 histogram
axs[2].hist(post_2000['oitc'].dropna(), bins=30, density=True, color='#A90D8B')  # no outlines
axs[2].axvline(post_2000_mean, color='black', linestyle='--', linewidth=1.5)  # Add mean line
axs[2].set_title('Histogram of OITC for Buildings Built Post 2000')
axs[2].set_xlabel('OITC')
axs[2].set_ylabel('Percent within vintage')  # Changed to Density for clarity with density=True
axs[2].set_ylim(0, 0.25)
axs[2].yaxis.set_major_formatter(PercentFormatter(xmax=1,decimals = 0))  # Set y-axis as percentage
...
#oitc by climate zone
df_grouped_cz = df0.groupby('in.ashrae_iecc_climate_zone_2004')['oitc'].mean().reset_index()

# Create the bar chart
plt.figure(figsize=(10, 6))
plt.barh(df_grouped_cz['in.ashrae_iecc_climate_zone_2004'], df_grouped_cz['oitc'], color='skyblue')
plt.ylabel('Climate Zone')
plt.xlabel('Average OITC')
plt.title('Average OITC by Climate Zone')
plt.xlim(20, 35)
...
#scatterplot of electricity use vs oitc
plt.figure(figsize=(10, 6))
plt.scatter(df0['oitc'], df0['out.electricity.net.energy_consumption.kwh'], color='blue', alpha=0.6)
plt.xlabel('OITC')
plt.ylabel('Electricity Net Electricity Consumption (kWh)')
plt.title('Scatter Plot of Net Electricity Consumption vs OITC')
plt.grid(True)
...
#scatterplot of overall energy consumption vs oitc
plt.figure(figsize=(10, 5))
plt.scatter(df0['oitc'], df0['out.site_energy.total.energy_consumption.kwh'], color='#1200C3', alpha=0.6)
plt.xlabel('OITC')
plt.ylabel('Total Site Energy Consumption (kWh)')
plt.title('Scatter Plot of Net Energy Consumption vs OITC')
plt.grid(True)
...
# # Example geocode format: 'G0600530'
# # Extract state and county FIPS codes from 'in.county'
# df0['state_fips'] = df0['in.county'].str[1:3]
# df0['county_fips'] = df0['in.county'].str[3:6]
# df0['geocode'] = df0['state_fips'] + df0['county_fips']
# #map oitc by county
# # Calculate the average OITC for each county
# county_oitc_avg = df0.groupby('geocode')['oitc'].mean().reset_index()

# # Load the U.S. county shapefile
# # Make sure to download and provide the correct path to the shapefile
# shapefile_path = 'path_to_shapefile/cb_2018_us_county_500k.shp'
# counties = gpd.read_file(shapefile_path)

# # Merge the shapefile data with the average OITC data
# counties['geocode'] = counties['STATEFP'] + counties['COUNTYFP']
# merged = counties.merge(county_oitc_avg, on='geocode', how='left')

# # Plotting the heatmap
# fig, ax = plt.subplots(1, 1, figsize=(15, 10))
# merged.boundary.plot(ax=ax, linewidth=1)
# merged.plot(column='oitc', ax=ax, legend=True, cmap='viridis', missing_kwds={"color": "white"})

# # Set plot title and remove axis
# plt.title('Average OITC by County')
# plt.axis('off')
# plt.show()
